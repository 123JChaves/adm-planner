...
export default function EditEmpresa() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const emp = resEmp.data.empresa || resEmp.data;
                setNome(emp.nome || "");
                setCnpj(emp.cnpj || "");
                
                const log = emp.logradouro;
                if (log) {
                ...
                setError("Erro ao sincronizar dados da empresa.");
            }
        };
        if (id) fetchData();
    }, [id]);
    ...
    // Se o usuário apagar o campo (texto vazio)
    if (!texto.trim()) {
        setId(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await instancia.put(`/empresa/${id}`, {
                nome, cnpj,
                logradouro: {
                    id: idLogradouro, nome: textoLogradouro,
                    numero: Number(numero), latitude, longitude,
                    bairro: {
                        id: idBairro, nome: textoBairro,
                        cidade: {
                            id: idCidade, nome: textoCidade,
                            estado: {
                                id: idEstado, nome: textoEstado,
        ...
        <div className="flex flex-col min-h-screen bg-gray-200 text-black">
            ...
            <div className="flex-1 px-4 py-6 max-w-5xl mx-auto w-full">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <span>
                    <Link href="/empresa/list" className="bg-cyan-500 text-white px-2 py-1 
                    me-1 rounded-md hover:bg-cyan-600">Listar</Link>
                     <Link href={`/empresa/${id}`} className="bg-blue-500
                    me-1 text-white px-2 py-1 me-1 rounded-md hover:bg-blue-600">Visualizar</Link>
                    </span>
                </div>
                ...
                <form onSubmit={handleSubmit} className="mt-6 bg-white shadow-md rounded-lg p-8 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    ...
                    <div className="flex flex-col">
                        ...
                        <input 
                            ...
                            onChange={(e) => { setNome(e.target.value); if (erros.nome) setErros(p => ({...p, nome: ""})); }}
                    ...
                    <div className="flex flex-col">
                        ...
                        <input 
                            ...
                            readOnly
                            onChange={(e) => { setCnpj(e.target.value); if (erros.cnpj) setErros(p => ({...p, cnpj: ""})); }}
                            className={`border p-2 rounded outline-none transition-all focus:ring-2 ${erros.cnpj ? 'border-red-500 bg-red-50 focus:ring-red-200' : 'border-gray-300 focus:ring-blue-300'}`}
                        />
                    ...
                    <div className="flex flex-col">
                        ...
                        <input 
                            ...
                            onChange={(e) => { setNumero(e.target.value); if (erros.numero) setErros(p => ({...p, numero: ""})); }}
                    ...
                    <div className="flex flex-col">
                        ...
                        <input
                            ...
                            onChange={(e) => { setLatitude(e.target.value); if (erros.latitude) setErros(p => ({ ...p, latitude: "" })); }}
                    ...
                    <div className="flex flex-col">
                        ...
                        <input
                            ...
                            onChange={(e) => { setLongitude(e.target.value); if (erros.longitude) setErros(p => ({ ...p, longitude: "" })); }}
                    ...
                    <button 
                        ...
                        className="md:col-span-2 bg-orange-500 text-white font-bold py-3 rounded-md hover:bg-orange-600 transition-colors mt-4"
                    >
